...
selected_phenotype = st.sidebar.selectbox("**Select a phenotype**", ["Length at birth", "Weight at birth"], index=0)

# Selection menu for month
selected_month = st.sidebar.selectbox("**Select a month**", ["Month 4", "Month 5", "Month 6", "Month 7", "Month 8+", "All time steps"], index=0)
...
with st.container():
    ...
    with col14:
        ...
        if selected_phenotype == 'Weight at birth':
            phenotype_title = 'Weight'
        elif selected_phenotype == 'Length at birth':
...
col1, col2, col3, col4, col5 = st.columns([20, 20, 20, 3, 37])
...
with st.container():
    with col1:
        ...
        elif selected_phenotype == "Length at birth": ### BIRTH LENGTH
            if selected_month == "Month 4":
...
# Boxplots (Hospitalized)
    ...
    # Define the line chart with CI for length
    line_with_ci_length = alt.Chart(df_grouped_length).mark_line().encode(
        ...
        strokeDash=alt.StrokeDash('condition', scale=alt.Scale(domain=['no_nvp', 'nvp', 'hospitalized_Pnvp'], range=[[10], [0], [5]]))  # Different line styles for each condition
    )
    ...
    band_with_ci_length = alt.Chart(df_grouped_length).mark_errorband(extent='ci', opacity=0.1).encode(
        ...
        y='ci_lower:Q',
        y2='ci_upper:Q',
        color=alt.Color('condition:N', scale=alt.Scale(domain=['no_nvp', 'nvp', 'hospitalized_Pnvp'], range=list(custom_colors.values())), legend=alt.Legend(title='Condition', orient='bottom'))  # Include legend for error band
    )

    chart_with_ci_band_length = (band_with_ci_length + line_with_ci_length).properties(
        width="container",  # Adjust chart width
        height=500  # Adjust chart height
    ).interactive()  # Make the chart interactive (zoom, pan, etc.)

    # Define the line chart with CI for weight
    ...
    # Define a function to create the chart without the 95% CI
    ...
    # Display the chart based on checkbox input
    with col5:
        # Display radio button options for viewing different stats
        stats_view = st.radio(
        ...
        if selected_phenotype == 'Length at birth':
            if stats_view == "95% CI":
                st.altair_chart(chart_with_ci_band_length, use_container_width=True)  # Render the length chart with the 95% CI band
            else:
                st.altair_chart(chart_length_no_ci, use_container_width=True)  # Render the length chart without the 95% CI band

        elif selected_phenotype == 'Weight at birth':
            if stats_view == "95% CI":
                st.altair_chart(chart_with_ci_band_weight, use_container_width=True)  # Render the weight chart with the 95% CI band
            else:
                st.altair_chart(chart_weight_no_ci, use_container_width=True)

# Section for displaying differences between boys and girls
...
with st.container():
    with col20:
        #### No NVP
        if selected_phenotype == "Length at birth":
            if selected_month == "All time steps":
                histo4_1 = create_histo(source4_1h, color1, "No NVP (n=" + str(sum4_1) + ")")
                st.altair_chart(histo4_1, use_container_width=True)      
                histo4_2 = create_histo_w(source4_2wh, color2, "NVP (n=" + str(sum4_2) + ")")
                st.altair_chart(histo4_2, use_container_width=True)
